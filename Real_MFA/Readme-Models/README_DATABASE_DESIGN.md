...
├─────────────────────────────────────────────────┤
│  ┌──────────────┬──────────────┬──────────────┐ │
│  │ ACCOUNTS APP │   OTP APP    │NOTIFICATIONS│ │
│  │              │              │    APP       │ │
│  └──────────────┴──────────────┴──────────────┘ │
│  ┌──────────────────────────────────────────┐   │
│  │         AUDIT_LOG APP                    │   │
...
otp (MFA)
    ├── Notifications (Send OTP codes)
    └── audit_log (Track OTP events)

notifications (Communication)
    └── audit_log (Track notification events)

audit_log (Auditing)
    └── (No dependencies, read-only from others)
```
...
└────────────┬──────────────────────────────────┬─┘
    ...
    ┌──────────────────┐            ┌──────────────────┐
    │  PasswordHistory │            │   MFAChangeLog   │
    │ (Old passwords)  │            │ (MFA audit trail)│
    └──────────────────┘            └──────────────────┘
```
...
└──────────┬────────────────────────────┬──────────┘
    ...
    │               │          │                │
    │ - access_jti  │          │ - fingerprint  │
    │ - refresh_jti │          │ - device_type  │
    │ - expires_at  │          │ - os           │
    │ - status      │          │ - browser      │
    └───────┬───────┘          └────────┬───────┘
...
└──────┬───────────────────┘
    ...
    ┌──▼──────────────┐       ┌──────────────────┐
    │ OTP             │       │ TOTPDevice       │
    │ (One-time codes)│       │ (TOTP secret)    │
    │                │       │                  │
    │ - code_hash    │       │ - secret_base32  │
    │ - expires_at   │       │ - backup_codes   │
    │ - used_at      │       └──────────────────┘
    │ - attempt_count│
    └────────────────┘       ┌──────────────────┐
                             │ BackupCode       │
    ┌──────────────────┐     │ (Recovery codes) │
    │ MFAChallenge     │     │                  │
    │ (Active MFA)     │     │ - code_hash      │
    │                │     │ - used_at        │
    │ - session_id    │     │ - used_by_ip     │
    │ - challenge_id  │     └──────────────────┘
    │ - expires_at    │
    └──────────────────┘     ┌──────────────────┐
                             │ MFARecovery      │
...
└──────┬───────┘
       ...
       │                              │
    ┌──▼──────────────────┐  ┌────────▼──────────┐
    │NotificationPreference│  │NotificationConsent│
    │ (Global settings)    │  │ (GDPR consent)    │
    └─────────────────────┘  └───────────────────┘
       │ OneToMany
       │
    ┌──▼──────────────────────────────┐
    │ DetailedNotificationPreference   │
    │ (Per-notification-type settings) │
    │                                  │
    │ - notification_type (14 types)   │
    │ - email_enabled                  │
    │ - sms_enabled                    │
    │ - in_app_enabled                 │
    │ - push_enabled                   │
...
┌────────────────────────────────┐
│  AuditLogSummary (Daily)       │
│ - user                         │
│ - date (UNIQUE with user)      │
│ - total_sessions               │
│ - total_devices                │
│ - high_risk_events             │
...
Year 2: 50 GB (1M users × 2 events/year growth)
...
Load balancing:
  Write → Primary
  Read (session checks) → Primary
  Read (reports) → Replica 1
...
# ❌ BAD: Loading all columns unnecessarily
...
✅ **44 Tables** across 4 apps
✅ **150+ Indexes** for performance  
✅ **44 GB/year** storage at 1M users
✅ **Scales to 10M+** users with partitioning
